...
def sleep_candidates(baby_id, start_local, end_local):
    ...
    /hds/me/v11/, /cs/me/v11/). The old /ss/ sleep-session service is gone
    from this host -- every path under it 404s. What does exist is the
    journal service:

        /cs/me/v11/babies/{id}/journals/grouped-tracking?group=...

    which answers 400 {"name":"InvalidDataError","message":"invalid group
    name"} for a group it doesn't recognise, so the endpoint is right and
    only the group name is in question. 'activity' is known to return diaper
    and breastfeeding entries; sleep is presumably a sibling group. We try
    the plausible spellings and log what each returns.

    Set SNOO_SLEEP_PATH (a full URL) and SNOO_SLEEP_GROUP to pin the answer
    once it is known and skip the probing.
    """
    pinned_url = os.environ.get("SNOO_SLEEP_PATH")
    pinned_group = os.environ.get("SNOO_SLEEP_GROUP")
    ...
    window = {"fromDateTime": frm, "toDateTime": to}
    cs = f"{HB_API}/cs/me/v11/babies/{baby_id}"

    if pinned_url:
        params = dict(window)
        if pinned_group:
            params["group"] = pinned_group
        return [(pinned_url, params)]

    groups = [
        "sleepSession", "sleep_session", "sleepSessions", "session",
        "sessions", "snoo", "snooSession", "sleeps", "napping", "nap",
        "activity", "all", "summary",
    ]
    cands = [(f"{cs}/journals/grouped-tracking", dict(window, group=g))
             for g in groups]

    # No group at all: some validators answer with the list of valid values.
    cands.append((f"{cs}/journals/grouped-tracking", dict(window)))

    # Other plausible homes for sleep under the same service.
    cands += [
        (f"{cs}/journals", dict(window)),
        (f"{cs}/journals", dict(window, type="sleep")),
        (f"{cs}/sessions", dict(window)),
        (f"{cs}/sessions/aggregated", {"startTime": naive}),
        (f"{cs}/sleep/aggregated", {"startTime": naive}),
        (f"{cs}/sleep-sessions", dict(window)),
    ]
    return cands


def type_inventory(body):
    """Distinct 'type' values in a response -- tells us at a glance whether
    sleep entries are in there at all."""
    seen = set()
...
def fetch_day(session, hdrs, baby_id, start_local, end_local, remembered,
              tz, soothing):
    """Tries the candidates and returns (endpoint, json) for the one that
    actually yields sleep blocks, falling back to the first plain 200.

    Once an endpoint has produced sleep on an earlier day of this run it is
    tried first and short-circuits, so the probing only costs us on the first
    day (and only until SNOO_SLEEP_PATH is pinned).
    """
    ...
    for url, params in cands:
        ...
        label = url.replace(HB_API, "")
        if params.get("group"):
            label += f"?group={params['group']}"

        if resp.status_code != 200:
            log(f"  {label} -> {resp.status_code} {preview(resp, 220)}")
            continue

        try:
            body = resp.json()
        except ValueError:
            log(f"  {label} -> 200 but not JSON: {preview(resp, 200)}")
            continue

        kinds = type_inventory(body)
        blocks = extract_blocks(body, tz, soothing)
        log(f"  {label} -> 200 types={kinds or '[]'} "
            f"sleep_blocks={len(blocks)} {preview(resp, 400)}")
...
def main():
    ...
    for back in range(lookback, -1, -1):
        ...
        if blocks:
            ...
            remembered = endpoint
        log(f"  => {len(blocks)} sleep blocks from {endpoint.replace(HB_API, '')}")
